...
- **(3) 不完全な履歴ゆえに不明な複合リスク** → 拾える DEFER が残らない。実装に下ろすと次の3つに分解され、いずれも DEFER にならない:
...
FRAMEWORK 章 (3)「不完全な履歴ゆえに不明な複合リスク」は、実装に下ろすと (i) 履歴窓の不備・
(ii) 起きていない事実を待つ・(iii) 既知悪性パターンへの接近 に分解され、いずれも DEFER に
ならない（本節冒頭の (3) 参照）。このうち (iii) は DEFER でなく危険性軸（composite risk）に
振り分けた。その受け皿だった #100 は、**射程外として close した**。
...
### DEFER 後の扱い: 解決機構は決定論的にエスカレーションする（#135）

決定層（入口）は「DEFER するか」を判定するだけで、「その曖昧さが何で、どう解決できるか」は
DEFER 後の解決機構（`DeferralResolver`、当初 #89・出口側は #135 に移管）に委ねる。入口と解決の
分離は AARM が入口をクリーンに保つ境界切り分けに沿う（`docs/aarm/deferral.md` §3）。

**現状の DEFER 発生源はいずれも、解決時点で正当に扱える新しい情報を持たない。** laarma の実装で
DEFER を生む経路は3つに限られる:

- **(A) R3(b) 由来**: `PolicyEngine._resolve_priority()` の同一 priority 競合。文脈 (a, C) を
  追加収集しても消えない——同じ (a, C) を同じルールエンジンに再度かければ同じ矛盾が再現するだけで、
  これはポリシー設計側の欠陥であり、DEFER 解決の対象にできる性質の不確実性ではない。
- **(B) `production_delete_defer` 由来**: 本番かつメンテナンス窓外という条件。窓が開くまで
  実時間で待てば理論上は解消しうるが、laarma は `proxy.call()` が同期的に一発で返る単発実行
  モデルであり、保留中アクションを永続化して後で再開する機構を持たない。加えて
  `docs/design/environment-demo-fiction.md`（#110）が、メンテナンス窓を含む環境条件を汎用の
  評価入力として定式化せず**デモフィクション**として明示的に踏襲する方針を確定済みであり、
  これを解消するための汎用環境監視・待機基盤を新設することは、その確定判断と整合しない。
- **(C) `defer_low_confidence` 由来**: 意図の曖昧さ。曖昧さの解消には本当に新しい情報
  （明確化された意図、追加のユーザー入力）が要るが、現行実装は DEFER 後に新しい情報を何も
  収集していない（内容収集ワークフローは未実装）。

3つとも、`resolve()` が呼ばれる時点で LLM が正当に扱える新しい情報を持たない。ここで LLM に
ALLOW/DENY/STEP_UP の decision を出させることは、決定層から LLM decision 判定を除去した
#112 の原則（「LLM は signal を出す、decision はポリシーが決める」）を DEFER 解決層だけ破る
ことになる。かつ実測でも、新情報が無い中で LLM が既存の文脈を読み直した結果、DEFER された
未実行アクションを「既に実行済み」と誤読する事実誤認バグを踏んだ（#166）。入力データ自体の
誤りは `_recent_actions_excluding()`（`context_accumulator.py`）で是正済みだが、それでも
「同じ情報を LLM にもう一度読ませて decision を出させる」構造自体は残っていた。

**結論（#135）**: `DeferralResolver.resolve()` から LLM 呼び出しを完全に除去し、DEFER は
常に決定論的に STEP_UP へエスカレーションする。(A)(B)(C) を区別する分岐は持たない——
どの発生源でも結論が同じ（新情報が無いので人間に委ねる）ため、SDK 側でプロジェクト固有の
ルール ID を知る必要も生じない（`policy.yaml` は SDK 外で管理という層分離を保てる）。

これは「DEFER 後に自律解決を試みる」という体裁を否定するものではない。呼び出し元
`tool_proxy.py` が「自律解決を試みる...」と表示してから `resolve()` を呼ぶ流れ自体は、
`environment-demo-fiction.md` が環境条件について採った立場——**体裁は残すが、それを汎用の
実能力の主張とは取り違えない**——と同型でよい。問題だったのは体裁ではなく、その体裁の裏で
LLM の出力が実際の decision を左右し、LLM が誤読すれば意図した着地（STEP_UP）を踏み外して
ALLOW/DENY に転びうる構造だった。

**この決定に含まれないもの**: 将来、本物の内容収集機構（ユーザーへの追加質問・別エージェントに
よる補完等）が実装された場合は話が変わる。その場合も LLM が直接 decision を出すのではなく、
収集した新情報を既存の signal 抽出層（`confidence_llm.py`・`action_matches_intent_llm.py`）に
再度通し、その結果を `PolicyEngine` に再度かけて π(a, C') を決定論的に評価する、という形で
DEFER 解決層にも「LLM は signal、decision はポリシー」の原則を一貫させるのが筋になる。これは
現状スコープ外（内容収集機構自体が未実装）であり、着手時に再設計する。confidence が曖昧さを
正しく低く算出できるかという較正は #77（射程は明確化済みだが実施しない方針）。
...
データセットと、それに対する体系的な閾値探索が要る。laarma は本番投入を目指さない学習用の
試作実装であり（冒頭「この文書の位置づけ」参照）、そのようなデータセットを用意して統計的較正を
行う計画は無い。
