...
**設計上の含意**: ただし R3 の (a)(b)(c) *だけ* を DEFER の実装根拠にすると、FRAMEWORK 章の「矛盾する文脈信号」「複合リスクの不明さ」を実装が取りこぼす。規格ドクトリンが「規範条項と informative 記述の両方を読め」と要求するのはまさにこのためで、DEFER 設計は両章を突き合わせて行う必要がある（laarma での統合は [../design/decision-layer-policy-engine.md](../design/decision-layer-policy-engine.md) §4 の領分）。なお #3「不完全な履歴ゆえの複合リスク」が (a)(b)(c) に対応しないのは、有限のプレフィックス（それまでの履歴）からは複合リスクを確定できない——ランタイム検証でいう monitorable でない性質＝判定保留（inconclusive）——という側面もある（[research/](research/) 参照）。
