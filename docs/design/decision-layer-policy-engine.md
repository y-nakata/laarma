...
- **条件1〜15 の状況（全15条件の移植完了、#142）**: 条件2・3・5・6・7・8・13・14・12前半は明示
  ...
  という原則。原文は例示として「破壊/書込」を挙げる）は ALLOW ルール化の対象外のまま #128 で
  検討していたが、条件2 のツール制限撤廃（`read_file`/`list_files` のみ `none_of` 除外、#161）に
  よって実現された（#165 での訂正・クローズ済み）。原文の「破壊/書込」は旧 IntentAlignment の
  ファイル操作中心のドメインに基づく例示であり、原則そのものはツール型のゲートではなく「risk が
  materialize しない無害な準備ステップ（`read_file`/`list_files` 等の情報収集）では介入せず、
  実際にリスクが生じるアクションで介入する」というタイミング・対象選定の原則である。`webhook`
  （外部送信）・`database`（機微データへのクエリ）はファイルの削除・書込ではないが、それ自体が
  risk が materialize するアクションであり、条件2 が非破壊ツールにも及ぶことはこの原則の正しい
  一般化であって原則違反ではない（旧条件2 の `destructive_tools`/`write_file` 限定は、当時の
  `action_matches_intent` ヒューリスティックの精度不足による暫定的な AS-IS の制約であり、原則が
  要求する適用範囲そのものではなかった）。条件1（単一信号に写像不可な意味論的
