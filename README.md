...
| R3 意図整合性評価 | MUST | ⚠️ | `PolicyEngine.evaluate()` が式(3)の π を単一関数として実装（全マッチ収集 → priority 解決）。δ（semantic_distance・data_classification・confidence・scope_expansion・action_matches_intent）を参照する match predicate を実装済みで、意図整合性 DENY・STEP_UP・ctx-ALLOW の各ルールが実際に発火する。R3(a)（未 populate な context 参照 → DEFER）は laarma の同期評価モデルでは構造的に発生しないため未実装。R3(b)（同一 priority 競合 → DEFER）は実装済み。R3(c)（confidence 低下 → DEFER）は実装済みだが、情報収集系ツールを対象から除外し適用範囲を限定している。詳細は [docs/design/decision-layer-policy-engine.md](docs/design/decision-layer-policy-engine.md) |
