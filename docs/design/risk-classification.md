...
laarma は、アクションの危険性を**固定のツールリスク等級**として持たない。代わりに、**アクセスしたデータと文脈から動的に把握する**。これを担うのは次の2つである。
...
## 3. 設計判断: なぜ「固定のツールリスク等級」を持たないか

laarma は、ツールごとに `READ_ONLY` / `WRITE` / `DESTRUCTIVE` のような**静的なリスク等級を割り当てる型を持たない**。理由は次の通り。

- **危険性は文脈依存である。** 同じツールでも、触れるデータ（PII か public か）や、ユーザー意図との整合性によって、危険性は変わる。固定等級は「ツールを見れば危険度が決まる」という前提に立つが、その前提は AARM の文脈依存評価（R3）と相容れない。仕様の δ（data_classification など）は、まさにこの「文脈からの動的把握」を要求している。
- **固定等級は二重管理になる。** ツールに等級を貼ると、ツールが増えるたびに等級も付与せねばならず、付け忘れ・誤付与という失敗モードを生む。data_classification はキーワードと集合からアクションごとに導出するので、ツール定義側に等級メタデータを持たせる必要がない。
- **リスク等級の用途は、より適切な仕組みに分解できる。** 「破壊的か」は `destructive_tools` 集合（policy 上書き可）、「機密データに触れるか」は data_classification の動的ラベル、「意図に沿うか」は IntentAlignment、と関心ごとに分かれている。単一の等級型に押し込めるより、関心の分離が保たれる。

したがって laarma の現在の設計は、「ツールに固定の危険度ラベルを貼る」のではなく、「**アクセスしたデータと文脈からリスクを動的に把握する**」方向に寄せてある。リスク等級型を新たに導入する必要はない。
