...
「棄却後に不足を診断して情報取得」と同型。ただし laarma の現状（AS IS）には追加証拠を集める処理フローが
まだ無いため、この「証拠取得で救う」形を採るなら新規フローの実装が要る（AS IS の欠落を TO BE の制約に
しないための注記であって、証拠取得型を否定するものではない）。救済を情報取得に紐づける発想は、次節の
fail-open リスクを検証可能な裏づけで抑える一つの手段として位置づく。
...
## 3. 要注意: 「LLM に decision を出させて別の判定を覆す」入れ子と、confidence 加点の fail-open

**要点**: 分野が警告するのは「**LLM に decision（authorization の結論）そのものを出させ**、その出力で別の
判定を上書きする」という入れ子である。一方、**LLM が δ の一信号（confidence 等）を産出し、決定論ルールが
閾値で結論を出す**枠組みは、この入れ子には当たらない。両者を混同しないことが肝心。
...
**この警告が当たるもの / 当たらないものの区別**:
- **入れ子に当たる（避けるべき）**: LLM に ALLOW/DENY/STEP_UP という decision を直接出させ、それを結論に
  する、あるいはその出力で別の判定を覆す。#135 で観測した「`DeferralResolver` の LLM が、保留中で未実行の
  アクションを『実行済み』と誤読して根拠不成立の DENY を出した」事象は、まさに LLM に decision を出させた
  結果の失敗であり、この入れ子の実例。
- **入れ子に当たらない（枠組みとしては正当）**: LLM が confidence という δ の一信号に寄与し、decision は
  決定論ルール（confidence 閾値ルール）が出す。これは semantic distance 等の δ を算出して決定論で受けるのと
  同じ構造であり、confidence を LLM で**下げる（弾く）ことも上げる（加点する）ことも、構造としては同一**。
  「弾くのは可、足すのは不可」という非対称は、この枠組みの中では根拠を持たない。

**ただし confidence 加点には固有の注意がある（入れ子ではなく fail の向きの問題）**: LLM で confidence を
下げる誤りは DEFER/STEP_UP に倒れる（fail-close 側＝安全側に誤る）が、上げる（加点する）誤りは、本来
DEFER/DENY に落ちるべきものを閾値超えで通す（fail-open 側＝危険側に誤る）。この非対称は「入れ子かどうか」
ではなく「誤ったときどちらに転ぶか」の非対称である。したがって加点を用いるなら、fail-open を避ける歯止めを
設ける（例: 加点だけで DENY を覆させない、STEP_UP→ALLOW の緩和に留めて DENY→ALLOW の飛び越えを
許さない、加点の根拠を receipt に記録して検証可能にする）。前節の「証拠取得で救う」は、この検証可能性を
証拠で裏づける一つの手段。加えて、二軸判定が機能する前提として confidence が較正されている必要がある
（未較正のヒューリスティックでは、閾値を上げれば誤りが確実に減る、という単調性が保証されない）——#77 依存。

参照:
- Provably Secure Agent Guardrail（LLM に結論を出させる形の限界、決定論的下限の欠如）: https://arxiv.org/html/2605.29251
...
- **支持される形（軸の分離）**: 危険性軸（距離 = 危険信号）と評価可能性軸（confidence）を混同せず、別軸として
  扱う。距離は危険性側の判定に使い、confidence は評価可能性側（低ければ DEFER）に使う。両者を一つの値に
  丸めない。これは分離原則（§1）が支持する。distance / confidence とは別に意図整合性を独立信号として持つか
  （confidence を濁さないための素案）は #128 の保留論点。
- **confidence への LLM 寄与は可、ただし加点は歯止め付き（§3）**: LLM が confidence 信号に寄与し決定論が
  結論を出す枠組みは正当（「LLM に decision を出させる」入れ子ではない）。下げる（弾く）も上げる（加点）も
  枠組みは同一。ただし加点は fail-open 側に誤るため、加点だけで DENY を覆させない・緩和は STEP_UP 止まり・
  根拠を receipt に記録、等の歯止めを設ける。
- **避けるべき形**: LLM に decision（ALLOW/DENY/STEP_UP）そのものを出させて結論にする、または別判定を
  覆す（§3 の入れ子。#135 の実例）。および、危険性と評価可能性を一つの値に混ぜて丸めること（§1 の分離に反する）。
- **前提**: いずれにせよ二軸判定が機能するには confidence が較正されている必要がある（#77 依存）。

本調査の中心的な発見: 警告されるのは「LLM に decision を出させる入れ子」であって、「LLM が confidence 信号に
寄与し決定論が結論を出す」枠組みではない。後者は下げるも上げるも構造同一で、加点は入れ子ではなく
fail-open リスクゆえ歯止め付きで用いる。具体的にどの priority 帯・どの match 条件・どの信号構成で実装するかは
#128 / #129 で確定する。
